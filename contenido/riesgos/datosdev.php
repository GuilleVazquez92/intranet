...
require( CONTROLADOR . 'riesgos.php');
	...
	foreach ($deuda as $key) {
		...
		$cuota_total += $key['cuota'];
		$deuda_total += $key['total'];
	}
...
// TELEFONO ------------------------------------
	...
	if ($datos['telefono'] == 00001 || $datos['telefono'] == '00001') {
	$puntos += 3;
	}
	if ($datos['telefono'] == 00010 || $datos['telefono'] == '00010') {
	$puntos += 1;
	}
	if ($datos['telefono'] == 00011 || $datos['telefono'] == '00011') {
	$puntos += 2;
	}
	if ($datos['telefono'] == 00100 || $datos['telefono'] == '00100') {
	$puntos += 2;
	}
...
//CLIENTE RECURRENTE ------------------------------------
	...
	if($datos['cliente'] == 'CLIENTE'){
		...
		$saldo_cuota = (($datos['ingreso']*$endeudamiento)-$cuota_total)+50000;
		$capacidad = ($datos['ingreso']*$endeudamiento)+50000;
		$limite_prestable = $capacidad*16;
		$saldo_total = $limite_prestable - $deuda_total;
		...
		// echo "cuenta:".$datos['mora_interna']."<br>";
		// echo "puntos:".$puntos ."<br>";
		// echo "cuota:".$capacidad."<br>";
		// echo "saldo Cuota:".$saldo_cuota."<br>";
		// echo "limite:".$limite_prestable ."<br>";
		// echo "saldo total:".$saldo_total ."<br>";

		$envio->cuenta = $datos['cuenta'];
		$envio->linea = $limite_prestable;
		$envio->cuota = $capacidad;
		$envio->saldo_cuota = $saldo_cuota;
		$envio->saldo_total = $saldo_total;
		$envio->deuda_mensual = $cuota_total;
		$envio->deuda_total = $deuda_total;
