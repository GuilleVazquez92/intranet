...
try{
	...
	operacion ope,
	trim(split_part(split_part(aadocu,'-',1),' ',1)) cedula,  -- ver para limpiar
	becta cuota, 
	...
	and fsd015.cufech = current_date-1
	union

	SELECT 	lote.lote, 
	operacion ope,
	split_part(trim(fsd0011.aadocu),'-',1) cedula,  -- ver para limpiar
	becta cuota, 
	fsd015.cumonn recibo,
	fsd015.cufech fecha,
	round(bkcapi) capital,
	...
	and bkcaje='PRONET' 
	and case 
	when date_part('dow',current_date) = 1 then fsd015.cufech = current_date-4
	when date_part('dow',current_date) = 2 then fsd015.cufech = current_date-4
	when date_part('dow',current_date) = 3 then fsd015.cufech BETWEEN current_date-4 AND current_date-2
	when date_part('dow',current_date) = 4 then fsd015.cufech = current_date-2
	when date_part('dow',current_date) = 5 then fsd015.cufech = current_date-2
	end
	union
	SELECT 	lote.lote, 
	operacion ope,
	...
	and case 
	when date_part('dow',current_date) = 1 then fsd015.cufech = current_date-4
	when date_part('dow',current_date) = 2 then fsd015.cufech BETWEEN current_date-4 AND current_date-2
	when date_part('dow',current_date) = 3 then fsd015.cufech = current_date-2
	when date_part('dow',current_date) = 4 then fsd015.cufech = current_date-2
	when date_part('dow',current_date) = 5 then fsd015.cufech = current_date-2
	end
	order by 6,1,2;";
	...
	$init->Lotepgo->BoletaBanco	= ''; //normalizar($row["BoletaBanco"]),
