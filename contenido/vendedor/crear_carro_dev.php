	...
	<div class="contenedor_carro" id="contenedor_carro">
		...
		<div class="card card-body">
			<div class="input-group" id="forma_pago">
				...
				<div class="input-group-append d-none" id="tipo_venta_change">
					<button class="btn btn-sm btn-outline-warning" type="button" onclick="forma_pago_change();">Eliminar</button>
				</div>
			</div>
			<small class="form-text text-danger d-none" id="tipo_venta_help">Si usted elimina el carro, deberá cargar todos los datos nuevamente.</small>
		</div>
		...
		<div class="card card-body d-none" id="detalle_carro">
			<button type="button" class="btn btn-sm btn-block btn-outline-warning mb-2" onclick="lista_prod();">
		...
		<div class="d-none" id="pie">
			<div class="d-none" id="tipo_credito_pie">
				<!-- Cantidad de cuotas -->	
				<p class="px-2 py-1 my-1 bg-secondary text-white rounded" id="id_cant_cuota">Cantidad de cuotas</p>
				<div class="card card-body">
					<div class="input-group input-group-sm">
						<input  type="number" class="form-control" id="cant_cuota" value="0" min="1" max="16" step="1" onchange="">
					</div>
				</div>
				...
				$fecha_final = date("Y-m-d",strtotime($fecha_inicial."+ 5 days"));
				?>	
				<p class="px-2 py-1 my-1 bg-secondary text-white rounded" id="id_primer_vto">Primer Vencimiento</p>
				<div class="card card-body">			
					<input type="date" class="form-control" name="primer_vto" id="primer_vto" min="<?= $fecha_inicial ?>" max="<?= $fecha_final?>" value="<?= $fecha_inicial?>" onchange="validar_fecha()">
				</div>
				...
				<div class="card card-body">
					<div class="input-group input-group-sm">
						<input type="number" class="form-control" id="entrega" name="entrega" value="0" onchange="">
					</div>
				</div>	
			</div>
...
function primer_pago(){
	...
	var primer_pago = document.getElementById('primer_pago');
	var primer_vto  = document.getElementById('primer_vto');

	var date 	  = new Date();
	var rango_ini = new Date();
	var rango_min = new Date();
	var rango_max = new Date();

	var fecha_ini;	
	var fecha_min;
	var fecha_max;

	if (primer_pago.value) {

		if(primer_pago.value==1){
			rango_ini.setDate(date.getDate() + 2);
			rango_min.setDate(date.getDate() + 1);
			rango_max.setDate(date.getDate() + 7);

		}else{
			rango_ini.setDate(date.getDate() + 31);
			rango_min.setDate(date.getDate() + 21);
			rango_max.setDate(date.getDate() + 46);
		}

		fecha_ini = rango_ini.toISOString().split('T')[0];
		fecha_max = rango_max.toISOString().split('T')[0];
		fecha_min = rango_min.toISOString().split('T')[0];

		document.getElementById('primer_vto').value = fecha_ini;
		document.getElementById('primer_vto').min 	= fecha_min;
		document.getElementById('primer_vto').max 	= fecha_max;
...
function cargar_carro(){
	...
	switch (tipo_venta) {
		case '1':
			...
			$.ajax({
				type:'POST',
				url:"http://intranet.facilandia.com.py/api/v2/index.php/carritos",
